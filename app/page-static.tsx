      ...
      <div className="w-full text-center py-16" style={{ backgroundColor: '#F5F3F4' }}>
        <div className="max-w-4xl mx-auto px-4">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight" style={{ color: '#2c73ff' }}>
            {t('hero.title') || 'EL MUNDO, UNA PANTALLA.'}
          </h1>
          <p className="text-lg md:text-xl mb-8 leading-relaxed" style={{ color: '#161A1D' }}>
            {t('hero.description') || 'Contenido que conecta. Noticias que informan. Música que inspira. Deportes que unen. Cine, podcasts y radios que acompañan. Todo sucede aquí, en vivo.'}
          </p>
          <a
          ...
          >
            {t('hero.createChannel') || 'Crea tu canal'}
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg sm:text-2xl font-bold text-primary">
              {totalLiveChannels} {t('search.liveChannels') || 'live channels'}
            </h2>
            {!loading && !error && (
          ...
          {error && (
            <div className="text-center py-12">
              <p className="text-red-500 text-lg mb-4">{t('channels.error') || 'Error'}: {error}</p>
              <button
                onClick={refetch}
                className="bg-button text-primary px-4 py-2 rounded-md hover-bg-button transition-colors"
              >
                {t('channels.retry') || 'Retry'}
              </button>
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              ...
              {channels.length === 0 && !loading && (
                <div className="text-center py-12">
                  <p className="text-disabled text-lg">{t('channels.notFound') || 'No channels found'}</p>
                  {searchTerm && (
