...
function HomePageContentTranslated() {
  const router = useRouter();
  const searchParams = useSearchParams();
  ...
  return (
    <Suspense fallback={
      ...
      <ClientOnly fallback={<HomePageContent />}>
        <HomePageContentTranslated />
      </ClientOnly>
    </Suspense>
  );
}

// Comentario: Página principal optimizada para build de Vercel - Cursor
// - Forzado renderizado dinámico para evitar timeouts
