...
export default function EmailPage() {
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setStep('otp');
        setMessage('Código de verificación enviado al nuevo email');
      } else {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        setMessage(data.error || 'Código de verificación inválido');
      }
    } catch (error) {
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-card rounded-lg p-6 border border-border-primary">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-primary">Cambiar Email</h1>
            <p className="text-muted">Actualiza tu dirección de email con verificación OTP</p>
          </div>
        </div>
      </div>
      ...
      {/* Current Email */}
      <div className="bg-card rounded-lg p-6 border border-border-primary">
        <h2 className="text-lg font-semibold text-primary mb-4">Email Actual</h2>
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted">Email actual de tu cuenta</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card rounded-lg p-6 border border-border-primary">
        <h2 className="text-lg font-semibold text-primary mb-4">
          {step === 'email' ? 'Nuevo Email' : 'Verificar Código'}
        </h2>
        
        {message && (
          <div className={`mb-4 p-3 rounded-lg ${
            message.includes('correctamente') || message.includes('enviado')
          ...
          <form onSubmit={handleVerifyOTP} className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted mt-1">
                Ingresa el código de 4 dígitos enviado a {newEmail}
              </p>
            </div>
      ...
      {/* Security Info */}
      <div className="bg-card rounded-lg p-6 border border-border-primary">
        <h2 className="text-lg font-semibold text-primary mb-4">Información de Seguridad</h2>
        <div className="space-y-3">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <p className="text-sm text-primary font-medium">Verificación por OTP</p>
              <p className="text-sm text-muted">Se enviará un código de 4 dígitos al nuevo email para verificar la propiedad</p>
            </div>
          </div>
          <div className="flex items-start space-x-3">
            <svg className="w-5 h-5 text-[#2c73ff] mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
              <p className="text-sm text-primary font-medium">Seguridad</p>
              <p className="text-sm text-muted">El cambio de email requiere verificación para proteger tu cuenta</p>
            </div>
          </div>
        </div>
      </div>
